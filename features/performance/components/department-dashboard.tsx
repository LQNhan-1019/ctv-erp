...
import { Banknote, CalendarClock, ClipboardCheck, FileWarning, Gauge, ReceiptText, RefreshCw, TrendingUp, Truck, UsersRound, WalletCards } from 'lucide-react';
...
import { ComparisonBars, ComparisonTable, MetricCard, ProgressDonut, TrendChart, type DashboardTone, type TableColumn } from './dashboard-widgets';
...
const definitions: Record<DepartmentDashboardKind, { code: DashboardCode; title: string; eyebrow: string; description: string; tone: DashboardTone; defaults: DashboardWidgetLayout[] }> = {
  sales: { code: 'SALES', title: 'Điều hành Kinh doanh', eyebrow: 'PHÒNG KINH DOANH TỔNG HỢP', description: 'Sản lượng, lãi gộp và hiệu suất khu vực, cửa hàng, đội xe.', tone: 'green', defaults: ['sales-kpis', 'sales-regions', 'sales-region-chart', 'sales-stores', 'sales-fleet'].map((widgetKey, index) => ({ widgetKey, visible: true, width: index === 0 ? 'FULL' : 'HALF' })) },
  accounting: { code: 'ACCOUNTING', title: 'Tài chính - Kế toán', eyebrow: 'PHÒNG TÀI CHÍNH KẾ TOÁN', description: 'Doanh thu, lợi nhuận, công nợ và tiến độ mục tiêu tài chính.', tone: 'blue', defaults: [{ widgetKey: 'finance-kpis', visible: true, width: 'FULL' }, { widgetKey: 'finance-trend', visible: true, width: 'TWO_THIRDS' }, { widgetKey: 'finance-goals', visible: true, width: 'THIRD' }] },
  'hr-admin': { code: 'HR_ADMIN', title: 'Hành chính Nhân sự', eyebrow: 'PHÒNG HÀNH CHÍNH NHÂN SỰ', description: 'Năng suất nhân sự và lối tắt tới các nghiệp vụ cần xử lý.', tone: 'violet', defaults: [{ widgetKey: 'hr-kpis', visible: true, width: 'FULL' }, { widgetKey: 'hr-progress', visible: true, width: 'HALF' }, { widgetKey: 'hr-actions', visible: true, width: 'HALF' }] },
};
...
export default function DepartmentDashboard({ kind }: { kind: DepartmentDashboardKind }) {
  ...
  const [month, setMonth] = useState(currentMonth); const [reload, setReload] = useState(0); const [data, setData] = useState<DashboardData | null>(null); const [loading, setLoading] = useState(true); const [error, setError] = useState('');
  const layout = useDashboardLayout(request, definition.code, definition.defaults);
  useEffect(() => { let active = true; getDepartmentDashboard(request, kind, month).then(value => { if (active) setData(value); }).catch((cause: Error) => { if (active) setError(cause.message); }).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, [kind, month, reload, request]);
  function refresh() { setLoading(true); setError(''); setReload(value => value + 1); }
  const metrics = useMemo(() => data?.metrics ?? [], [data]);
  const cards = (items: DashboardMetric[], tone: DashboardTone) => <div className="department-kpi-grid">{items.map((metric, index) => { const value = shown(metric); const icons = [WalletCards, TrendingUp, ReceiptText, Gauge]; return <MetricCard key={metric.metricId} label={metric.metricName} value={value.value} unit={value.unit} caption={metric.subjectName} progress={metric.progressPercent} icon={icons[index % icons.length]} tone={tone}/>; })}</div>;
  ...
  return <div className={`nova-account-page department-dashboard theme-${definition.tone}`}><header className="department-dashboard-hero"><div><p>{definition.eyebrow}</p><h1>{definition.title}</h1><span>{definition.description}</span></div><div><label><span>Kỳ báo cáo</span><input type="month" value={month} onChange={event => { setLoading(true); setError(''); setMonth(event.target.value); }}/></label><button onClick={refresh} disabled={loading}><RefreshCw size={17}/>{loading ? 'Đang tải' : 'Làm mới'}</button></div></header>
    {loading && <div className="performance-state"><span className="nova-session-spinner"/>Đang tải dashboard phòng ban…</div>}{error && <div className="performance-error"><Icon name="alert"/>{error}<button onClick={refresh}>Thử lại</button></div>}{!loading && !error && <DashboardCanvas layout={layout} blocks={blocks}/>}</div>;
}

function salesBlocks(metrics: DashboardMetric[], cards: (items: DashboardMetric[], tone: DashboardTone) => React.ReactNode) {
  ...
  return {
    'sales-kpis': <Panel eyebrow="CHỈ SỐ TRỌNG YẾU" title="Kết quả kinh doanh trong kỳ">{cards(metrics.filter(item => item.scopeLevel === 'COMPANY').slice(0, 4), 'green')}</Panel>,
...
function accountingBlocks(metrics: DashboardMetric[], cards: (items: DashboardMetric[], tone: DashboardTone) => React.ReactNode, trend: { label: string; revenue: number; grossProfit: number; netProfit: number; target: number | null }[]) {
  return {
    'finance-kpis': <Panel eyebrow="TỔNG HỢP TÀI CHÍNH" title="Các chỉ số cần theo dõi">{cards(metrics, 'blue')}</Panel>,
...
function hrBlocks(metrics: DashboardMetric[], cards: (items: DashboardMetric[], tone: DashboardTone) => React.ReactNode, average: number) {
  ...
  return {
    'hr-kpis': <Panel eyebrow="TỔNG QUAN HCNS" title="Năng suất và kết quả trong kỳ">{cards(metrics, 'violet')}</Panel>,
    'hr-progress': <Panel eyebrow="TIẾN ĐỘ" title="Mức hoàn thành công việc"><div className="department-progress-summary"><ProgressDonut value={Number.isFinite(average) ? average : null} label="Tiến độ bình quân" tone="violet"/><div><Gauge/><b>{metrics.length} chỉ số đang theo dõi</b><span>Dữ liệu lấy từ bảng cập nhật công việc hằng ngày.</span></div></div></Panel>,
