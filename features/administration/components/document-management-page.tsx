...
export default function DocumentManagementPage() {
  ...
  if (!canView) return <section className="nova-access-denied"><span><Icon name="shield" /></span><p>KHÔNG ĐỦ QUYỀN</p><h1>Bạn không thể xem hồ sơ và thời hạn.</h1><div>Cần quyền <code>ADMIN.DOCUMENT.VIEW</code>.</div></section>;
  return <div className="nova-account-page admin-module-page">
    <header className="nova-page-header admin-module-header"><div><p className="nova-eyebrow">TUÂN THỦ & PHÁP LÝ</p><h1>Quản lý thời hạn giấy tờ</h1><span>Theo dõi bảo hiểm, PCCC, giấy phép, kiểm định và hợp đồng theo từng phòng ban/đơn vị.</span></div><div className="admin-header-actions">{canImport && <button className="nova-button secondary" onClick={() => setImportOpen(true)}><Icon name="upload" />Import Excel</button>}{canManage && <button className="nova-button primary" onClick={() => setEditing(null)}><Icon name="plus" />Thêm giấy tờ</button>}</div></header>
    ...
    {!loading && tab === 'types' && <section className="admin-data-card"><header><div><h2>Danh mục loại giấy tờ</h2><p>Quy định nhóm và số ngày cảnh báo mặc định.</p></div>{canManage && <button className="nova-button primary" onClick={() => setTypeEditing(null)}><Icon name="plus" />Thêm loại</button>}</header><div className="admin-category-grid">{types.map((item) => <article key={item.id} className={!item.active ? 'inactive' : ''}><div><small>{item.code}</small><b>{item.name}</b><span>{categoryLabels[item.category]} · nhắc trước {item.defaultReminderDays} ngày</span></div>{canManage && <div className="admin-row-actions"><button onClick={() => setTypeEditing(item)}><Icon name="edit" /></button><button className="danger" onClick={() => void removeType(item)}><Icon name="trash" /></button></div>}</article>)}</div></section>}
