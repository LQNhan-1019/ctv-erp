...
export default function AttendanceDeviceManagementPanel({ request, sources, employees }: {
  ...
  return <section className="attendance-panel attendance-device-manager">
    <header><div><h2>Quản lý máy chấm công từ xa</h2><p>Chọn nhân viên đã ánh xạ ID chấm công để tạo hoặc cập nhật hồ sơ trên máy.</p></div>
      <button className="nova-button primary" disabled={busy || !selectedConnectionId || !selected.size} onClick={() => void push()}><Icon name="upload" />{busy ? 'Đang nạp…' : `Nạp vào máy (${selected.size})`}</button>
    </header>
    ...
    {!devices.length && <div className="nova-info-note"><Icon name="alert" /><span>Chưa có kết nối máy chấm công đang hoạt động. Hãy tạo kết nối trong Cấu hình hệ thống.</span></div>}
    <div className="nova-info-note"><Icon name="shield" /><span>ERP chỉ gửi mã chấm công, mã nhân viên và họ tên. Mật khẩu thiết bị được đọc từ biến môi trường ở backend và không xuất hiện trên trình duyệt.</span></div>
    {error && <div className="performance-error"><Icon name="alert" />{error}</div>}
    {result && <div className={result.failed ? 'performance-error' : 'performance-success'}><Icon name={result.failed ? 'alert' : 'check'} />{result.message}</div>}
