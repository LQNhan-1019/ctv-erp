...
export default function ExpenseManagementPage() {
  ...
  if (!canView) return <section className="nova-access-denied"><span><ShieldCheck /></span><p>KHÔNG ĐỦ QUYỀN</p><h1>Bạn không thể xem chi phí hành chính.</h1><div>Cần quyền <code>ADMIN.EXPENSE.VIEW</code>.</div></section>;
  return <div className="nova-account-page admin-module-page">
    <header className="nova-page-header admin-module-header"><div><p className="nova-eyebrow">NGÂN SÁCH HÀNH CHÍNH</p><h1>Tổng hợp chi phí hành chính</h1><span>Quản lý dữ liệu theo mẫu báo cáo: đơn vị, bộ phận, hạng mục, nhóm chi phí, dự kiến, thực tế và hóa đơn.</span></div><div className="admin-header-actions">{canExport && <button className="nova-button secondary" disabled={busy === 'export'} onClick={() => void exportWorkbook()}><Download />{busy === 'export' ? 'Đang xuất…' : 'Xuất Excel'}</button>}{canImport && <button className="nova-button secondary" onClick={() => setImportOpen(true)}><Upload />Import Excel</button>}{canManage && <button className="nova-button primary" onClick={() => setEditing(null)}><Plus />Thêm chi phí</button>}</div></header>
    ...
    {!loading && tab === 'categories' && <section className="admin-data-card"><header><div><h2>Nhóm và phân nhóm chi phí</h2><p>Danh mục chuẩn hóa từ file báo cáo tháng 09/2026.</p></div>{canManage && <button className="nova-button primary" onClick={() => setCategoryEditing(null)}><Plus />Thêm nhóm</button>}</header><div className="admin-category-grid">{categories.map((item) => <article key={item.id} className={!item.active ? 'inactive' : ''}><div><small>{item.code}</small><b>{item.name}</b><span>{item.parentName || 'Nhóm cấp 1'}</span></div>{canManage && <div className="admin-row-actions"><button onClick={() => setCategoryEditing(item)}><Pencil /></button><button className="danger" onClick={() => void removeCategory(item)}><Trash2 /></button></div>}</article>)}</div></section>}
