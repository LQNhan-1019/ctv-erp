...
export default function AdministrativeImportDialog({ open, type, request, download, close, imported }: { open: boolean; type: 'EXPENSE' | 'DOCUMENT'; request: Request; download: Download; close: () => void; imported: (message: string) => Promise<void> }) {
  ...
  if (!open) return null;
  return <div className="nova-overlay"><section className="nova-dialog administrative-import-dialog"><header><div><p>NHẬP DỮ LIỆU EXCEL</p><h2>Import {label}</h2><span>Không cần cột ID: hệ thống tự sinh ID cho từng bản ghi khi import thành công.</span></div><button onClick={close}><Icon name="x" /></button></header><div className="nova-dialog-body">
    <div className="admin-import-guide"><div><Icon name="download" /><span><b>Chưa có mẫu?</b><small>Tải file mẫu; chỉ điền dữ liệu và mã danh mục, không nhập ID.</small></span><button className="nova-button secondary" disabled={busy === 'template'} onClick={() => void template()}>Tải mẫu</button></div><div><Icon name="upload" /><span><b>Đã có file?</b><small>Hệ thống nhận cột tiếng Việt, ngày dạng dd/MM/yyyy và tự sinh ID bản ghi.</small></span></div></div>
    ...
    {error && <div className="nova-form-error">{error}</div>}{file && <div className="admin-import-file"><span><Icon name="table" /></span><div><b>{file.name}</b><small>{(file.size / 1024).toFixed(1)} KB</small></div><button className="nova-button primary" disabled={!encoded || busy === 'preview'} onClick={() => void inspect()}>Xem trước</button></div>}
