...
export default function AttendanceImportDialog({ open, request, download, sources, close, onImported }: { open: boolean; request: Request; download: Download; sources: AttendanceSource[]; close: () => void; onImported: (result: AttendanceImportResult) => Promise<void> }) {
  ...
  if (!open) return null;
  return <div className="nova-overlay"><section className="nova-dialog attendance-import-dialog"><header><div><p>NHẬP DỮ LIỆU EXCEL</p><h2>Import chấm công</h2><span>Xem trước và kiểm tra toàn bộ dữ liệu trước khi ghi vào hệ thống.</span></div><button onClick={close}><Icon name="x" /></button></header><div className="nova-dialog-body">
    <div className="attendance-import-choices"><article><span><Icon name="download" /></span><div><b>Chưa có file import mẫu?</b><p>Tải mẫu chuẩn, điền mỗi nhân viên–mỗi ngày một dòng rồi tải lại tại bước bên dưới.</p></div><button type="button" className="nova-button secondary" disabled={busy === 'template'} onClick={() => void downloadTemplate()}>Tải file mẫu</button></article><article><span><Icon name="upload" /></span><div><b>Đã có file dữ liệu?</b><p>Chọn file hiện có; hệ thống tự tìm sheet và nhận diện tên cột Việt/Anh.</p></div></article></div>
    ...
    {file && <div className="attendance-import-file"><Icon name="table" /><div><b>{file.name}</b><small>{(file.size / 1024).toFixed(1)} KB</small></div><button className="nova-button primary" disabled={!encoded || busy === 'preview'} onClick={() => void inspect()}><Icon name="eye" />{busy === 'preview' ? 'Đang kiểm tra…' : 'Xem trước dữ liệu'}</button></div>}
  ...
  </div><footer><button className="nova-button secondary" onClick={close}>Hủy</button><button className="nova-button primary" disabled={!preview || preview.invalidRows > 0 || busy === 'import'} onClick={() => void commit()}><Icon name="upload" />{busy === 'import' ? 'Đang import…' : `Import ${preview?.validRows ?? 0} dòng`}</button></footer></section></div>;
}
