...
export default function AttendanceManagementPage() {
  ...
  if (!canView) return <section className="nova-access-denied"><span><ShieldCheck /></span><p>KHÔNG ĐỦ QUYỀN</p><h1>Bạn không thể xem chấm công.</h1><div>Cần permission <code>HR.ATTENDANCE.VIEW</code>.</div></section>;
  return <div className="nova-account-page attendance-page">
    <header className="nova-page-header attendance-header">
      ...
      <div className="attendance-actions"><input aria-label="Tháng" type="month" value={month} onChange={(event) => setMonth(event.target.value)} />{canImport && <button className="nova-button secondary" onClick={() => setImportOpen(true)}><Upload />Import Excel</button>}{canExport && <button className="nova-button primary" disabled={busy === 'export'} onClick={() => setExportOpen(true)}><Download />Xuất Excel</button>}</div>
    </header>
...
function SyncPanel({ request, sources, employees, canSync, busy, execute }: { request: Request; sources: AttendanceSource[]; employees: EmployeeOption[]; canSync: boolean; busy: string; execute: (label: string, action: () => Promise<unknown>) => Promise<void> }) {
  ...
  async function clear() { if (!selectedId || !await appDialog.confirm(`Xóa toàn bộ log AMIS từ ${from} đến ${to}?`)) return; await execute('delete-data', () => deleteAttendanceData(request, 'AMIS', selectedId, from, to)); }
  ...
  return <section className="attendance-panel attendance-sync"><header><div><h2>Nguồn dữ liệu AMIS</h2><p>Đồng bộ theo ID chấm công đã ánh xạ; giờ vào/ra là lượt đầu tiên và cuối cùng trong ngày. Import Excel vẫn nằm ở nút phía trên.</p></div></header>{!canSync ? <p>Bạn chỉ có quyền xem, không có quyền đồng bộ.</p> : <><div className="attendance-sync-grid"><label><span>Kết nối AMIS</span><select value={selectedId} onChange={(event) => setSourceId(event.target.value)} required><option value="">Chọn kết nối AMIS</option>{filteredSources.map((source) => <option key={source.id} value={source.id}>{source.name} · {source.code}</option>)}</select></label><label><span>Từ ngày</span><input type="date" value={from} onChange={(event) => setFrom(event.target.value)} required /></label><label><span>Đến ngày</span><input type="date" value={to} onChange={(event) => setTo(event.target.value)} required /></label></div>{!filteredSources.length && <div className="nova-info-note"><TriangleAlert /><span>Chưa có kết nối AMIS chấm công đang hoạt động.</span></div>}<div className="attendance-sync-actions"><button className="nova-button primary" disabled={!selectedId || busy === 'sync'} onClick={() => void run(false)}><RefreshCw />Đồng bộ AMIS</button><button className="nova-button secondary" disabled={!selectedId || busy === 'sync'} onClick={async () => { if (await appDialog.confirm('Dữ liệu trong khoảng chọn sẽ được xóa và lấy lại. Tiếp tục?')) void run(true); }}><RefreshCw />Xóa & đồng bộ lại</button><button className="nova-button danger" disabled={!selectedId || busy === 'delete-data'} onClick={() => void clear()}><Trash2 />Xóa dữ liệu đã nạp</button></div><section className="attendance-identifier-panel"><header><div><h3>Ánh xạ ID chấm công → nhân viên ERP</h3><p>ID AMIS là mã nguồn riêng, không thay thế mã nhân viên.</p></div></header><form onSubmit={saveMapping}><input value={externalId} onChange={(event) => setExternalId(event.target.value)} placeholder="ID chấm công" required /><select value={employeeId} onChange={(event) => setEmployeeId(event.target.value)} required><option value="">Chọn nhân viên ERP</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.employeeCode} · {employee.fullName}</option>)}</select><button className="nova-button secondary" disabled={!selectedId || busy === 'mapping'}><Save />Lưu ánh xạ</button></form><div className="attendance-identifier-list">{identifiers.map((item) => <article key={item.id}><code>{item.externalAttendanceId}</code><span>→</span><div><b>{item.employeeName}</b><small>{item.employeeCode}</small></div><button className="danger" aria-label="Xóa ánh xạ" onClick={async () => { if (await appDialog.confirm('Xóa ánh xạ này?')) void execute('mapping-delete', () => deleteAttendanceIdentifier(request, item.id)).then(refreshIdentifiers); }}><Trash2 /></button></article>)}{selectedId && !identifiers.length && <p>Chưa có ánh xạ cho nguồn này.</p>}</div></section></>}</section>;
}
