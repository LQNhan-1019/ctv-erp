...
import { Check, Database, Download, RefreshCw, Server, ShieldCheck, X, type LucideIcon } from 'lucide-react';
...
const destinationMeta: Record<BackupDestination, { label: string; description: string; icon: LucideIcon }> = {
  ...
  return (
    <div className="nova-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="nova-dialog nova-backup-dialog" role="dialog" aria-modal="true" aria-labelledby="run-backup-title">
        <header><div><p>SAO LƯU THỦ CÔNG</p><h2 id="run-backup-title">Tạo bản sao dữ liệu</h2><span>Backend xuất SQL và mã hóa trước khi chuyển file tới đích đã chọn.</span></div><button onClick={onClose} aria-label="Đóng"><X /></button></header>
          ...
          <div className="nova-dialog-body">
            <fieldset className="nova-destination-options">
              ...
              {(Object.keys(destinationMeta) as BackupDestination[]).map((value) => {
                ...
                return <label key={value} className={destination === value ? 'selected' : ''}><input type="radio" name="destination" value={value} checked={destination === value} onChange={() => setDestination(value)} /><span><meta.icon /></span><div><b>{meta.label}</b><small>{meta.description}</small></div><em><Check /></em></label>;
              })}
            </fieldset>
            ...
            <div className="nova-backup-security-note"><span><ShieldCheck /></span><div><b>Mã hóa đầu cuối ở backend</b><p>File SQL được mã hóa AES-256-GCM. Khóa mã hóa chỉ đọc từ biến môi trường và không được lưu trong database hay trình duyệt.</p></div></div>
            {error && <div className="nova-form-error" role="alert">{error}</div>}
          </div>
          <footer><button type="button" className="nova-button secondary" onClick={onClose}>Hủy</button><button type="submit" className="nova-button primary" disabled={submitting || (destination !== 'DOWNLOAD' && compatible.length === 0)}><Database />{submitting ? 'Đang sao lưu…' : 'Bắt đầu sao lưu'}</button></footer>
        </form>
      </section>
    </div>
  );
...
export default function BackupsPage() {
  ...
  if (!canView) {
    return <section className="nova-access-denied"><span><ShieldCheck /></span><p>KHÔNG ĐỦ QUYỀN</p><h1>Bạn không thể xem lịch sử sao lưu.</h1><div>Cần permission <code>SYSTEM.BACKUP.VIEW</code> để truy cập chức năng này.</div></section>;
  }
  ...
  return (
    <div className="nova-account-page nova-backups-page">
      <header className="nova-page-header">
        ...
        {canRun && <button className="nova-button primary" onClick={() => setRunOpen(true)}><Database />Tạo bản sao lưu</button>}
      </header>

      <section className="nova-backup-hero">
        <div><span><ShieldCheck /></span><div><p>BẢO VỆ DỮ LIỆU</p><h2>SQL được mã hóa trước khi rời backend</h2><small>AES-256-GCM · kiểm tra toàn vẹn SHA-256 · không lưu khóa trong database</small></div></div>
        <dl>
      ...
      <section className="nova-account-panel">
        <header className="nova-backup-table-heading"><div><p>LỊCH SỬ SAO LƯU</p><h2>{totalItems} tác vụ</h2></div><button onClick={() => void load()} aria-label="Làm mới" disabled={loading}><RefreshCw className={loading ? 'spin' : ''} /></button></header>
          ...
          {!loading && jobs.map((job) => {
            ...
            return (
              <div className="nova-backup-row" role="row" key={job.id}>
                <span className="nova-backup-file"><i><Database /></i><div><b>{job.encryptedFileName || 'Đang tạo file…'}</b><small>{formatDate(job.startedAt)}</small></div></span>
                <span className="nova-backup-destination"><meta.icon /><div><b>{meta.label}</b><small>{job.connectionId ? job.connectionId.slice(0, 8) : 'Trình duyệt'}</small></div></span>
                ...
                <span>
                  ...
                  {job.downloadAvailable && canDownload && <button className="nova-button secondary compact" onClick={() => void downloadFile(job)} disabled={downloadingId === job.id}><Download />{downloadingId === job.id ? 'Đang tải…' : 'Tải về'}</button>}
                  {!job.remoteWebUrl && !job.downloadAvailable && <small>Không có thao tác</small>}
                </span>
              </div>
            );
          })}
          {!loading && jobs.length === 0 && <div className="nova-account-empty"><span><Database /></span><b>Chưa có bản sao lưu</b><p>Tạo bản sao đầu tiên để thiết lập điểm khôi phục dữ liệu.</p></div>}
        </div>
