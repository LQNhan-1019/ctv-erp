...
import ShiftSchedulingPanel from './shift-scheduling-panel';
...
type Tab = 'overview' | 'timesheet' | 'shifts' | 'sync';
...
export default function AttendanceManagementPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [overview, sheet, shiftItems, employeeItems, sourceItems] = await Promise.allSettled([
        getAttendanceDashboard(request, month), getTimesheet(request, month), listShifts(request),
  ...
  }, [canExport, canImport, canManage, canSync, canView, month, request]);
  ...
  return <div className="nova-account-page attendance-page">
    ...
    <nav className="attendance-tabs">{([['overview', 'Tổng quan'], ['timesheet', 'Bảng công'], ['shifts', 'Ca & phân ca'], ['sync', 'Nguồn dữ liệu']] as const).map(([value, label]) => <button key={value} className={tab === value ? 'active' : ''} onClick={() => setTab(value)}>{label}</button>)}</nav>
    ...
    {!loading && tab === 'timesheet' && timesheet && <section className="attendance-panel">
      <header><div><h2>Bảng công tháng {Number(month.slice(5))}</h2><p>Nhấn vào ô có dữ liệu để xem giờ vào/ra và hiệu chỉnh.</p></div></header>
    ...
    {!loading && tab === 'shifts' && (canManage ? <ShiftSchedulingPanel request={request} shifts={shifts} employees={employees} busy={busy} execute={execute} /> : <section className="attendance-panel"><p>Bạn chỉ có quyền xem ca làm việc.</p></section>)}
