...
export default function ExpenseInvoicesDialog({ expense, canManage, close, changed }: { expense: Expense; canManage: boolean; close: () => void; changed: () => Promise<void> }) {
  ...
  async function remove(invoice: ExpenseInvoice) { if (!window.confirm(`Xóa file “${invoice.fileName}”?`)) return; setBusy(invoice.id); try { await deleteExpenseInvoice(request, expense.id, invoice.id); if (selected?.id === invoice.id) { if (previewUrl) URL.revokeObjectURL(previewUrl); setPreviewUrl(''); setSelected(undefined); } await load(); await changed(); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Không xóa được hóa đơn'); } finally { setBusy(''); } }
  return <div className="nova-overlay"><section className="nova-dialog admin-dialog invoice-dialog"><header><div><p>HÓA ĐƠN ĐÍNH KÈM</p><h2>{expense.expenseNo}</h2><span>{expense.workContent}</span></div><button onClick={close}><Icon name="x" /></button></header><div className="nova-dialog-body"><div className="invoice-toolbar"><span>{invoices.length} file trên Google Drive</span>{canManage && <label className="nova-button primary"><Icon name="upload" />{busy === 'upload' ? 'Đang tải…' : 'Thêm hóa đơn'}<input type="file" accept=".pdf,.jpg,.jpeg,.png,.webp,application/pdf,image/jpeg,image/png,image/webp" disabled={busy === 'upload'} onChange={(event) => void upload(event.target.files?.[0])} /></label>}</div>{error && <div className="nova-form-error">{error}</div>}<div className="invoice-layout"><div className="invoice-list">{busy === 'load' && <div className="admin-loading">Đang tải…</div>}{invoices.map((invoice) => <article key={invoice.id} className={selected?.id === invoice.id ? 'active' : ''}><button className="invoice-open" disabled={busy === invoice.id} onClick={() => void preview(invoice)}><Icon name="file" /><span><b>{invoice.fileName}</b><small>{size(invoice.fileSize)} · {new Date(invoice.createdAt).toLocaleString('vi-VN')}</small></span></button>{canManage && <button className="invoice-delete" onClick={() => void remove(invoice)} aria-label="Xóa hóa đơn"><Icon name="trash" /></button>}</article>)}{!busy && !invoices.length && <div className="admin-empty">Chưa có file hóa đơn.</div>}</div><div className="invoice-preview">{previewUrl && selected ? <object data={previewUrl} type={selected.contentType} aria-label={selected.fileName} /> : <div><Icon name="file" /><p>Chọn một hóa đơn để xem trực tiếp</p></div>}</div></div></div><footer><button className="nova-button secondary" onClick={close}>Đóng</button></footer></section></div>;
}
