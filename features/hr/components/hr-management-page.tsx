...
export default function HrManagementPage() {
  ...
  if (!canView) return <section className="nova-access-denied"><span><Icon name="shield" /></span><p>KHÔNG ĐỦ QUYỀN</p><h1>Bạn không thể xem hồ sơ nhân sự.</h1><div>Cần quyền <code>HR.VIEW</code>.</div></section>;
  ...
  return <div className="nova-account-page hr-page">
    <header className="nova-page-header hr-header">
      ...
      {canManage && <div className="hr-header-actions"><button className="nova-button secondary" onClick={() => setDepartmentDialog(null)}><Icon name="folder" />Thêm phòng ban</button><button className="nova-button primary" onClick={() => setEmployeeDialog(null)}><Icon name="plus" />Thêm nhân viên</button></div>}
    </header>

    <section className="hr-summary">
      <article><span><Icon name="users" /></span><div><small>TỔNG HỒ SƠ</small><b>{employees.length}</b></div></article>
      <article><span><Icon name="check" /></span><div><small>ĐANG LÀM VIỆC</small><b>{active}</b></div></article>
      <article><span><Icon name="folder" /></span><div><small>PHÒNG BAN</small><b>{departments.filter((item) => item.unitType === 'DEPARTMENT').length}</b></div></article>
      <article><span><Icon name="plug" /></span><div><small>ĐÃ GHÉP AMIS HR</small><b>{linked}</b></div></article>
      <article><span><Icon name="key" /></span><div><small>TÀI KHOẢN ĐÃ GẮN</small><b>{linkedAccounts}</b></div></article>
    </section>

    {canManage && <section className="hr-sync-panel">
      <div><span><Icon name="refresh" /></span><div><b>Đồng bộ AMIS Nhân sự</b><small>Cơ cấu tổ chức → vị trí công việc → nhân viên → ghép log chấm công</small></div></div>
      ...
      <div className="hr-sync-actions"><button className="nova-button danger" disabled={!sourceId || Boolean(busy)} onClick={() => void runDeleteStructure()}><Icon name="trash" />{busy === 'delete-amis-structure' ? 'Đang xóa…' : 'Xóa cơ cấu phòng ban'}</button><button className="nova-button secondary" disabled={!sourceId || Boolean(busy)} onClick={() => void runRefresh()}><Icon name="refresh" className={busy === 'refresh-amis' ? 'spin' : ''} />{busy === 'refresh-amis' ? 'Đang làm mới…' : 'Làm mới toàn bộ'}</button><button className="nova-button primary" disabled={!sourceId || Boolean(busy)} onClick={() => void runSync()}><Icon name="refresh" className={busy === 'sync' ? 'spin' : ''} />{busy === 'sync' ? 'Đang đồng bộ…' : 'Đồng bộ ngay'}</button></div>
    ...
    {tab === 'departments' && <section className="hr-department-grid">{departments.map((item) => <article key={item.id} className={!item.active ? 'inactive' : ''}><header><span><Icon name={item.unitType === 'COMPANY' ? 'database' : 'folder'} /></span><div><small>{item.code}</small><h2>{item.name}</h2></div>{canManage && item.unitType === 'DEPARTMENT' && <button onClick={() => setDepartmentDialog(item)}><Icon name="edit" /></button>}</header><dl><div><dt>Đơn vị cha</dt><dd>{item.parentName || 'Đơn vị gốc'}</dd></div><div><dt>Nhân sự hoạt động</dt><dd>{item.employeeCount}</dd></div><div><dt>Nguồn dữ liệu</dt><dd>{item.amisLinked ? 'AMIS HR' : 'Nội bộ'}</dd></div></dl><footer>{item.active ? 'Đang sử dụng' : 'Tạm dừng'}</footer></article>)}</section>}

    {employeeDialog !== undefined && <EmployeeDialog employee={employeeDialog} departments={departmentOptions} jobs={jobs} busy={busy === 'employee'} onClose={() => setEmployeeDialog(undefined)} onSave={saveEmployee} />}
...
function EmployeeDialog({ employee, departments, jobs, busy, onClose, onSave }: { employee: Employee | null; departments: Department[]; jobs: JobTitle[]; busy: boolean; onClose: () => void; onSave: (input: EmployeeInput) => Promise<void> }) {
  const [error, setError] = useState('');
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      setError('');
      await onSave({ employeeCode: value(data, 'employeeCode'), fullName: value(data, 'fullName'), businessUnitId: value(data, 'businessUnitId'), jobTitleId: nullable(data, 'jobTitleId'), gender: nullable(data, 'gender') as Employee['gender'], dateOfBirth: nullable(data, 'dateOfBirth'), workEmail: nullable(data, 'workEmail'), personalEmail: null, phone: nullable(data, 'phone'), hiredOn: value(data, 'hiredOn'), terminatedOn: status === 'TERMINATED' ? nullable(data, 'terminatedOn') : null, employmentStatus: status });
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Không thể lưu nhân viên'); }
  }
  return <div className="nova-overlay"><section className="nova-dialog hr-dialog"><header><div><p>HỒ SƠ NHÂN SỰ</p><h2>{employee ? 'Chỉnh sửa nhân viên' : 'Thêm nhân viên'}</h2><span>Mã nhân viên phải trùng mã dùng cho chấm công.</span></div><button onClick={onClose}><Icon name="x" /></button></header><form onSubmit={submit}><div className="nova-dialog-body">
    ...
    <div className="nova-form-grid"><label><span>Phòng ban</span><select name="businessUnitId" defaultValue={employee?.businessUnitId} required><option value="">Chọn phòng ban</option>{departments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label><span>Chức danh</span><select name="jobTitleId" defaultValue={employee?.jobTitleId ?? ''}><option value="">Chưa xác định</option>{jobs.filter((item) => item.active).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div>
  ...
  </div><footer><button type="button" className="nova-button secondary" onClick={onClose}>Hủy</button><button className="nova-button primary" disabled={busy}><Icon name="save" />{busy ? 'Đang lưu…' : 'Lưu nhân viên'}</button></footer></form></section></div>;
}
...
function DepartmentDialog({ department, departments, busy, onClose, onSave }: { department: Department | null; departments: Department[]; busy: boolean; onClose: () => void; onSave: (input: DepartmentInput) => Promise<void> }) {
  ...
  return <div className="nova-overlay"><section className="nova-dialog hr-dialog compact"><header><div><p>CƠ CẤU TỔ CHỨC</p><h2>{department ? 'Chỉnh sửa phòng ban' : 'Thêm phòng ban'}</h2></div><button onClick={onClose}><Icon name="x" /></button></header><form onSubmit={submit}><div className="nova-dialog-body"><label><span>Đơn vị cha</span><select name="parentId" defaultValue={department?.parentId ?? ''}><option value="">Công ty gốc</option>{departments.filter((item) => item.id !== department?.id).map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><div className="nova-form-grid"><label><span>Mã phòng ban</span><input name="code" defaultValue={department?.code} pattern="[A-Za-z0-9._-]+" required maxLength={32} /></label><label><span>Tên phòng ban</span><input name="name" defaultValue={department?.name} required maxLength={255} /></label></div><label className="nova-switch-label"><span>Trạng thái</span><div><input type="checkbox" name="active" defaultChecked={department?.active ?? true} /><b>Cho phép sử dụng</b></div></label>{error && <div className="nova-form-error">{error}</div>}</div><footer><button type="button" className="nova-button secondary" onClick={onClose}>Hủy</button><button className="nova-button primary" disabled={busy}><Icon name="save" />{busy ? 'Đang lưu…' : 'Lưu phòng ban'}</button></footer></form></section></div>;
}
