...
export default function AdministrativeImportDialog({ open, type, request, download, close, imported }: { open: boolean; type: 'EXPENSE' | 'DOCUMENT'; request: Request; download: DownloadFile; close: () => void; imported: (message: string) => Promise<void> }) {
  ...
  return <div className="nova-overlay"><section className="nova-dialog administrative-import-dialog"><header><div><p>NHẬP DỮ LIỆU EXCEL</p><h2>Import {label}</h2><span>Không cần cột ID: hệ thống tự sinh ID cho từng bản ghi khi import thành công.</span></div><button onClick={close}><X /></button></header><div className="nova-dialog-body">
    ...
    <GoogleSheetSourcePicker request={request} feature={type === 'EXPENSE' ? 'ADMIN_EXPENSE' : 'ADMIN_DOCUMENT'} disabled={Boolean(busy)} onResolved={useSheet} />
